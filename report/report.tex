...
% TODO:
% Snak om de forskellige metoder der er blevet brugt.
% Nævn gerne flere methoder der er blevet brugt
% Måske nævn allerede her de to papers af Song og Hunsberger.
...
% Frames learning (finding parameters of a model) as optimizing some objective function.
...
\begin{equation}
  -\log \mathcal{L}_{\theta}(\mathbf{Y} \mid \mathbf{X})= -\sum_{i=1}^{N} \log \frac{1}{\sigma \sqrt{2 \pi}} e^{-\frac{(y_i - \hat{y}_i)^{2}}{2 \sigma^{2}}} = \sum_{i=1}^{N} \frac{1}{2}(y_i - \hat{y}_i)^2 \frac{1}{\sigma^2} + \log\left( \sigma \sqrt{2\pi} \right).
\end{equation}
If all constant terms are dropped it becomes apparent that minimizing the mean squared error, $\frac{1}{N}\sum_{i=1}^{N} (y_i - \hat{y}_i)^2$, performs maximum likelihood estimation.

% Talk about distribution assumpts (normal disitrbution), negative log likelihood and loss functions.

\subsection{Dataset}
% TODO:
% Skriv lidt mere meta omkring datasæt og hvordan de bruges i deep learning til en start.
...
\subsubsection{MNIST}
% Write specifically about MNIST here.

\subsubsection{CIFAR10}
% Write about CIFAR 10 dataset here.
...
It is possible to use artificial neural networks in classification tasks by transforming the output \textit{logits} into a discrete probability using a softmax vector function
\begin{equation}
  \text{softmax}(\mathbf{z})_i = \frac{e^{z_i}}{\sum_{j=1}^{K}{e^{z_j}}} \hspace{10pt} \text{for } i = 1,\ldots,K \text{ and } \mathbf{z} = (z_1, \ldots z_K) \in \mathbb{R}^K
\end{equation}
that scales every $z_i$ to lie in the range from 0 to 1 and makes the sum of all output probabilities equal to 1. The softmax function thereby satisfies the formal definition of probability mass functions. An important property of the softmax function is that the ordering of logits and there respective output probabilities stays the same. Eg. the largest value element of $\mathbf{z}$ also has the largest output probability. \\
By assuming that the output of the neural network is a categorical emperical distribution with density
\begin{equation}
  123
\end{equation}

% Softmax function -> Turns output logits into a distribution (sums to 1)
% Maximum likelihood estimation by optimization and loss functions
% - What is the problem with using squared loss?
% Cross-entropy
% One hot encoding
...
% TODO:
% - Skriv hvordan man kan få et predictive coding netværk til at implementere en softmax funktion.
...
% Write something about softmax and cross entropy
