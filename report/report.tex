...
The follow sections will describe alternative, more biologically plausible methods for training and evaluating artificial neural networks. It is the goal of these methods to satisfy the biological constraints put forward in section \ref{sec:biological-constraints}. These methods will primarily deal with the \textbf{the spiking problem} and \textbf{the weight transport problem}. 
...
\end{figure}
It is possible to derive the spike rate when $j>V_\text{th}$ by setting $v(t) = V_\text{th}$ and solving for $r = t^{-1}$ to get.
\begin{equation}
  r = \left[\tau_\text{ref} - \tau_{RC} \log \left(1-\frac{V_{\text {th }}}{j}\right) \right]^{-1}
\end{equation}
Notice that $\tau_\text{ref}$ is added to the time-to-spike, to incorporate the effect of the refractory period. \\
\\*
...
In order to input non-spiking variables into our SNN they should first be converted into spikes. This can be accomplished by poission rate coding. The generated spikes are sampled from a poission distribution  $X_i \sim \Pois(x_i)$.  
